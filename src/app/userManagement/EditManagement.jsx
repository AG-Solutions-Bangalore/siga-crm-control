...
const EditManagement = ({id,refetch,setIsViewExpanded}) => {
    ...
    const { data, isLoading: isFetching } = useQuery({
      ...
      queryFn: async () => {
        ...
        const response = await axios.get(`https://southindiagarmentsassociation.com/public/api/panel-fetch-usercontrol-by-id/${id}`, {
    ...
    const updateMutation = useMutation({
      ...
      onError: (error) => {
        toast({
          title: "Error",
          description: error.response?.data?.message || "Failed to update user control",
          variant: "destructive",
        });
      }
    });
    ...
    useEffect(() => {
      if (data?.usercontrol) {
        const userTypeArray = data.usercontrol.usertype ? data.usercontrol.usertype.split(',') : [];
    ...
    return (
      <div className="container mx-auto p-6">
        <h1 className="text-2xl font-bold mb-6 flex items-center">
          <CreditCardIcon className="mr-2" /> Edit Access Management
        </h1>
