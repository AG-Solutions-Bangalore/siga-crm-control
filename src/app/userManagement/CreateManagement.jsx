...
const CreateManagement = () => {
  ...
  // get the all page list from the Button compnents
  const pageOptions = [
    ...new Set(Object.values(ButtonComponents).map((btn) => btn.page)),
  ].map((page) => ({
    value: page,
    label: page,
  }));

  // filter the buttonn components according to page
  const buttonOptions = selectedPage
    ? Object.keys(ButtonComponents)
        .filter((key) => ButtonComponents[key].page === selectedPage)
        .map((key) => ({
          value: key,
          label: key,
        }))
      ...
      <div className="container mx-auto p-6">
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              <label className="block mb-2 text-sm font-medium">Page</label>
              <ShadcnSelect
                value={selectedPage}
